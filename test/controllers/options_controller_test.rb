...
class OptionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create option" do
    assert_difference('Option.count') do
      post options_url, params: { option: { option: @option.option, question_id: @option.question_id } }
    end
  ...
  test "should update option" do
    patch option_url(@option), params: { option: { option: @option.option, question_id: @option.question_id } }
